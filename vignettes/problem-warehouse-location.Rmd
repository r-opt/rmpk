...
The model with `rmpk` then looks like this:
...
x <- model$add_variable(x[i, j], i = 1:n, j = 1:m, type = "binary")
  
# 1 iff warehouse j is built
y <- model$add_variable(y[j], j = 1:m, type = "binary")
  
# maximize the preferences
model$set_objective(sum_expr(transportcost(i, j) * x[i, j], i = 1:n, j = 1:m) + 
...
model$add_constraint(x[i,j] <= y[j], i = 1:n, j = 1:m)
model
```
...
We solved the problem with an objective value of `r sprintf("%d", model$objective_value())`.
