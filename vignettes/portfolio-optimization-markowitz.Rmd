...
\begin{equation*}
\begin{array}{ll@{}ll}
\text{max}  & \displaystyle\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{n}C_{i,j} \cdot x_{i} \cdot x_{j} & &\\
...
model <- MIPModel(solver) %>% 
  add_variable(x[i], i = 1:n, lb = 0, ub = 1) %>% 
  set_objective(sum_expr(2 * C[i, j] * x[i] * x[j], i = 1:n, j = 1:n)) %>% 
  add_constraint(sum_expr(e[i] * x[i], i = 1:n) >= min_mu) %>% 
